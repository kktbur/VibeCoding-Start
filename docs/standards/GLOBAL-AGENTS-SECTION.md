...
- Establish a project-local `AGENTS.md` and an indexed documentation skeleton: `docs/INDEX.md`, `PRODUCT.md`, `PRD.md`, `ACCEPTANCE.md`, `CURRENT.md`, and `CODEMAP.md`.
- Use the `vibecoding-project-knowledge` Skill when available for INIT, UPDATE, and AUDIT. Keep document depth proportional to project complexity; do not omit the index or PRD because a project is small.
- Preserve intent, decisions, worklogs, raw sessions, evidence, and test artifacts locally. Curate active documents without irreversibly deleting raw history, and publish only redacted examples.
