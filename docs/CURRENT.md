...
- The v0.3.0 governance scope is recorded in [Plan 007](plans/007-v0.3.0-ecosystem-governance.md) and [ADR-0007](decisions/0007-v0.3.0-public-governance-surface.md); the recorded PR CI passed. A fresh-session retry using the packaged candidate in an Administrator-owned isolated empty Git project completed `INIT → UPDATE → AUDIT`: all required files were generated, the audits passed, raw evidence was ignored, the boundary README remained visible, and no commit or application source was created. No v0.3.0 merge, tag, or Release is claimed yet.
